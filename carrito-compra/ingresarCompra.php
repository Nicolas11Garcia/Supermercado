...
if (isset($_SESSION["nombre_usuario"]) == true){
    ...
    foreach ($lista as $k) { 
        ...
        $ingresando_al_detalle= $dao->agregarAlDetalle($numero_orden,$k->getId(),$k->getPrecioOferta(),$k->getCantidadEnCarrito());
...
//NO LOGIN
    ...
    foreach($_SESSION['id_productos_carrito'] as $key => $value){
        ...
        foreach ($lista_id_en_carrito as $k) {
            $ingresando_al_detalle= $dao->agregarAlDetalle($numero_orden,$k->getId(),$k->getPrecioOferta(),$_SESSION['cantidad'][$key]);
